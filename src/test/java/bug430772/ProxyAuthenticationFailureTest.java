...
public class ProxyAuthenticationFailureTest {
  ...
  public void testOkAuthenticator() throws Exception {
    ...
    Proxy proxy =
        new Proxy(Type.HTTP, new InetSocketAddress("127.0.0.1", connector.getLocalPort()));
    client.setProxy(proxy);
    client.setAuthenticator(auth);
    URL url = new URL("http://does.not.matter");
    HttpURLConnection connection = client.open(url);
    connection.connect();
    Assert.assertEquals(407, connection.getResponseCode());
  }
  ...
  public void startServer() throws Exception {
    ...
    server.setHandler(new AbstractHandler() {
      ...
      public void handle(String target, Request baseRequest, HttpServletRequest request,
        ...
        response.addHeader("Proxy-Authenticate", "Basic realm=\"Basic authentication\"");
